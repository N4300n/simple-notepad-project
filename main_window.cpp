...
main_window::main_window() {
    ...
    try {
        ...
        spell_highlighter = new highlighter(editor->document(), *checker);
    } catch (...) {
        QMessageBox::warning(this, "Dictionary Warning", "Could not load data/words.txt");
    }
    ...
    cursor_status_label = new QLabel(this);
    statusBar()->addPermanentWidget(cursor_status_label);
    ...
    connect(editor, &QTextEdit::cursorPositionChanged, this, &main_window::update_cursor_status);
...
void main_window::setup_format_menu() {
    ...
    connect(action_font, &QAction::triggered, this, &main_window::change_font);
    const auto* action_color = format_menu->addAction("Text Color...");
    connect(action_color, &QAction::triggered, this, &main_window::change_color);
}
...
void main_window::setup_tools_menu() {
    ...
    connect(action_check_spell, &QAction::triggered, this, &main_window::recheck_spelling);
}
...
void main_window::open_file() {
    ...
    try {
        QFile file(path);
        if (!file.exists()) { throw file_not_found_exception(path.toStdString()); }
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) { throw file_read_exception(path.toStdString()); }

        QTextStream in(&file);
        const auto contents = in.readAll();
        if (in.status() != QTextStream::Ok) { throw file_read_exception(path.toStdString()); }
...
void main_window::update_title() {
    if (current_file.isEmpty()) { setWindowTitle("Notepad"); }
    else { setWindowTitle("Notepad: " + current_file); }
}

void main_window::update_format_buttons() {
    const auto current = editor->currentCharFormat();
    if (bold_action != nullptr) bold_action->setChecked(current.fontWeight() == QFont::Bold);
    if (italic_action != nullptr) italic_action->setChecked(current.fontItalic());
    if (underline_action != nullptr) underline_action->setChecked(current.fontUnderline());
}
...
void main_window::show_find_replace_dialog() {
    if (!find_replace_dlg) {
        ...
        connect(find_replace_ui->find_next_button, &QPushButton::clicked,
            this, [this] { find_next(find_replace_ui->find_input->text(), make_find_flags(*find_replace_ui)); });
        connect(find_replace_ui->replace_button, &QPushButton::clicked,
            this, [this] { replace_current(find_replace_ui->find_input->text(), find_replace_ui->replace_input->text(), make_find_flags(*find_replace_ui)); });
...
void main_window::replace_current(const QString& term, const QString& replacement, const QTextDocument::FindFlags flags) const {
...
void main_window::replace_all(const QString& term, const QString& replacement, const QTextDocument::FindFlags flags) const {
    if (term.isEmpty()) return;
...
void main_window::show_context_menu(const QPoint& pos) {
    ...
    if (checker && !word.isEmpty() && !checker->is_correct(word.toStdString())) {
        menu->addSeparator();
        auto suggestions = checker->suggest(word.toStdString());
...
void main_window::recheck_spelling() {
    if (spell_highlighter) { spell_highlighter->rehighlight(); }
}